...
**Why multiple seeds:** a single seed proves nothing except "it worked once." `multi_seed_summary()` re-runs on 6 independently generated worlds (7 primary, plus 42, 13, 99, 2024, 555) and reports a win-rate, plus how many worlds actually contain a malformed invoice or a superseded promise. Agent B beats Agent A on rupees recovered in **6/6** of these worlds; Agent C matches or beats Agent B, the EV ablation, in **3/6** (5/6 before Phase E1 — see `docs/learning_findings.md` for why honest rung-setting cost it money); Agent D matches or beats Agent C, the learning ablation, in **4/6** (3 wins, 1 exact tie, 2 losses; it was 0/6 before Phase E2 measured `wait`) — all three numbers on the identical seed set, via `multi_seed_summary()`'s optional `primary_agent_ev` / `primary_agent_learned` arguments.
...
| E1 + E2 — EV sets the delivered rung; `wait` measured (post-deadline) | ✅ complete | `brain.ev_sets_rung` (label/execution gap 56% → 0 of 3,218 training SENDs); EV-chosen waits recorded and fitted (0 of 1,513 paid). Legacy setting reproduces the pre-E1 `results.json` to the paisa. Learned arm 0/6 → 4/6 (3 wins, 1 tie); hand-typed EV arm 5/6 → 3/6 — see `docs/learning_findings.md` |
| 11 — demo assets + video (CLAUDE.md's checklist item 11) | 🟡 in progress | Dashboard built (`scripts/build_dashboard.py` → `report/out/dashboard.html`, also `docs/index.html` for GitHub Pages) and submission packaging done (artifacts committed, README/ARCHITECTURE rewritten). Video script in ARCHITECTURE.md §11 — **not yet recorded.** |
| 12 — final check + submit (CLAUDE.md's checklist item 12) | ⬜ not started | Fresh-clone check + the submission form. |
...
10. Cash-flow intelligence (can this buyer pay, will they) and a next-best-action EV ranking ARE built (Phases 1–3), and there IS a real ablation (Phase 4): the same agent with that layer on beats itself with it off in 5 of the same 6 seeds, one honest loss included. A *learned* (not stated-assumption) probability model IS also built (`engine/learning.py`, a contextual bandit, Phases 8–16) and was wired into a fourth `agent+EV+learned` ablation arm — which first *lost* to the hand-typed EV grid on all 6 benchmark seeds (mean −₹22,53,175), root-caused to an untested `wait` value; after Phases E1/E2 (EV sets the rung sent; `wait` measured) it matches or beats it on 4/6 (3 wins, 1 tie, 2 losses), roughly break-even against the pre-E1 best. Both the EV layer and the learning layer ship **off** by default; a fresh clone reproduces the pre-learning agent exactly. What's genuinely still not built: message content that differs by the chosen negotiation action, reactive settlement-offer handling, a full strategy simulator, and — on the submission checklist — the demo video is not yet recorded (item 11's dashboard and packaging are done) and the submission form (item 12) is not yet filed.
