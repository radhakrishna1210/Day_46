...
| **Section 16:** on delay, buyer owes compound interest with monthly rests at 3× the RBI bank rate — automatically, even if the contract says no interest. | Compute exact ₹ interest owed to date. (Example: bank rate 6.5% → 19.5% p.a. compounded monthly. Keep the bank rate in config and verify the current value before submitting.) |
| **Section 43B(h), Income Tax Act:** if the buyer pays a registered MSME late, they cannot deduct that expense this financial year. | Compute the buyer's OWN tax cost of delaying. This flips the message from "please pay me" to "delaying costs YOU money." |
